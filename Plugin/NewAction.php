...
class NewAction
{
    ...
    private function getNofraudSettings() 
    {
        ...
        error_log("\n order place time manual capture check ".$apiUrl,3,BP."/var/log/new_action.log");
        try {
            $curl = curl_init();
            curl_setopt_array($curl, array(
                CURLOPT_URL => $apiUrl,
            ...
            error_log("\n order place time manual capture check ".$e->getMessage(),3,BP."/var/log/new_action.log");
        }
    }
}
?>
